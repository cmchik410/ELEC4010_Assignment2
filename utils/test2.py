...
def testing(model, data_loader, loss_fcn, metrics, encoder, device):
    ...
    test_acc_dict = {"pix": 0, "dice" : 0, "jaccard" : 0, "asd" : 0, "hd95" : 0}
    ...
    for i, data in enumerate(progBar, start = 1):
        ...
        test_acc_dict["pix"] = (test_acc_dict["pix"] * (i - 1) + acc) / i
        ...
        test_acc_dict["hd95"] = (test_acc_dict["hd95"] * (i - 1) + acc3) / i
        
        progBar.set_description("Testing: ")
        progBar.set_postfix(train_loss = test_loss, accuracy = test_acc_dict["pix"], dice = test_acc_dict["dice"], 
